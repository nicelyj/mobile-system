	...
	if($("#getTopMenu").val()=="true"){
		...
		$.post("/server/serverList.xml", "", function(xml){
			...
			$(xml).find("list").find("ServerList").each(function(){
				html+='<li title="'+$(this).find("serverDomainName").text()+' - '+ $(this).find("serverIp").text() +'('+$(this).find("serverPort").text()+')"><a href="/server/serverInfo.html?ServerListSeq='+$(this).find("serverListSeq").text()+'">'+$(this).find("serverName").text()+'</a></li>';
			});
